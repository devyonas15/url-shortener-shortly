...
public sealed class GetUrlByIdHandlerTest : TestFixture
{
    private static readonly Mock<IUrlRepository> UrlRepository = new();
    private readonly GetUrlByIdQueryHandler _handler = new(Mapper.Object, UrlRepository.Object);
    ...
    public async Task GivenInvalidId_ThenBadRequestException()
    {
        ...
        await Assert.ThrowsAsync<BadRequestException>(async () => await _handler.Handle(mockRequest, It.IsAny<CancellationToken>()));
    }
