...
public sealed class GenerateUrlCommandHandlerTest : TestFixture
{
    private static readonly Mock<IUrlRepository> UrlRepository = new();
    private readonly GenerateUrlCommandHandler _handler  = new(UrlRepository.Object, Mapper.Object);
    ...
    public async Task GivenValidLongUrl_WhenGenerateUrlCommand_AndDuplicateUrlFound_ThenThrowsDuplicateException()
    {
        ...
        await Assert.ThrowsAsync<DuplicateException>(async () => await _handler.Handle(request, CancellationToken.None));
    }
